...
public class Logger {
	...
	/*
	 * @Around(value = "declearJoinPointExpression()") public Object
	 * aroundMethod(ProceedingJoinPoint point) {
	 * 
	 * DataTools.WriteStringToFile2(logFilePath, DataTools.GetCurrDate()); Object
	 * result = null; String methodName = point.getSignature().getName();
	 * 
	 * try { // 前置通知
	 * 
	 * DataTools.WriteStringToFile2(logFilePath, "The method " + methodName +
	 * " start. param<" + Arrays.asList(point.getArgs()) + ">"); // 执行目标方法 result =
	 * point.proceed(); // 返回通知 DataTools.WriteStringToFile2(logFilePath,
	 * "The method " + methodName + " end. result<" + result + ">"); } catch
	 * (Throwable e) { // 异常通知 DataTools.WriteStringToFile2(logFilePath,
	 * "！！错误！！ : this method " + methodName + " end.ex message<" + e + ">"); throw
	 * new RuntimeException(e); } // 后置通知 DataTools.WriteStringToFile2(logFilePath,
	 * "The method " + methodName + " end."); return result; }
	 * 
	 */
